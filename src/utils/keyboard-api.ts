...
export class KeyboardAPI {
  ...
  async getTacxMacroCount(): Promise<number> {
  ...
  async setTacxMacroCount(count: number): Promise<void> {
    await this.hidCommand(APICommand.CUSTOM_MENU_SET_VALUE, [
      0x42,
      0x41,
      0,
  ...
  async getTacxMacroName(index: number): Promise<string> {
    const response = await this.hidCommand(APICommand.CUSTOM_MENU_GET_VALUE, [
      0x42,
      0x41,
      1,
      index,
  ...
  async setTacxMacroName(index: number, name: string): Promise<void> {
    const bytes = Array.from(name.slice(0, 12), (character) => {
  ...
  async setTacxKeyLabel(
    layer: number,
    row: number,
    col: number,
    label: string,
  ): Promise<void> {
    const bytes = Array.from(label.slice(0, 7), (character) => {
      const code = character.charCodeAt(0);
      return code >= 32 && code <= 126 ? code : 63;
    });
    await this.hidCommand(APICommand.CUSTOM_MENU_SET_VALUE, [
    ...
    await this.commitCustomMenu(0x42);
  }

  async saveTacxMacroMetadata(): Promise<void> {
    await this.commitCustomMenu(0x42);
  }
