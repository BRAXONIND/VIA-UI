...
export const MacroDetailPane: React.FC<Props> = (props) => {
  ...
  const selectWindowsApp = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        setQuickStatus({
          message: 'App picker works in local preview. Type the path here on web.',
          error: true,
        });
        return;
      }
      ...
      if (result.path) {
        ...
        setQuickStatus({message: 'App path selected.', error: false});
      }
    } catch {
      setQuickStatus({
        message: 'App picker works in local preview. Type the path here on web.',
            ...
            <LaunchSection>
              ...
              <LaunchRow>
                <LaunchInput
                  ...
                  value={launchCommand}
                  placeholder="Website URL, app name, path, or .exe"
                  spellCheck={false}
