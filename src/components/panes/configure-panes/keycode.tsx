...
import {getMacroCount, getMacroNames} from 'src/store/macrosSlice';
import {
...
const generateKeycodeCategories = (
  ...
  numMacros: number = 16,
  macroNames: string[] = [],
) => getKeycodes(numMacros, macroNames).concat(getOtherMenu(basicKeyToByte));
...
export const KeycodePane: FC = () => {
  ...
  const KeycodeCategories = useMemo(
    () => generateKeycodeCategories(basicKeyToByte, macroCount, macroNames),
    [basicKeyToByte, macroCount, macroNames],
  );
